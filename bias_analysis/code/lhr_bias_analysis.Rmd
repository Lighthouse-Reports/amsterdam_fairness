...
cms_model_comp <- cms_long %>%
  filter(Metric %in% c('PRED_P', 'FP', 'FDR', 'FPR'),
...
for(characteristic in unique(cms_model_comp$Feature_EN)){
...
### Viz
Just a codeblock to extract data for viz or make plots prettier
```{r}
cms_gender <- cms_long %>%
  filter(Feature == 'geslacht', Metric %in% c('TP', 'FP', 'TN', 'FN'), Dataset == 'Prepilot', Model == 'BR')
